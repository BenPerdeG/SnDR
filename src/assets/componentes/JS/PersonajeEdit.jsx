...
const PersonajeEdit = ({
    ...
    useEffect(() => {
        if (personaje) {
            ...
            // Obtener IDs de usuarios asignados a este personaje
            const usuariosAsignadosIds = (usuariosPersonaje || [])
                .filter(up => up.id_personaje === personaje.id)
                .map(up => up.id_usuario);

            // Obtener los objetos completos de usuario
            const asignados = (usuariosPartida || [])
                .filter(u => usuariosAsignadosIds.includes(u.id));
    ...
    useEffect(() => {
        ...
        const disponibles = (usuariosPartida || [])
            .filter(u => !usuariosAsignadosIds.includes(u.id));
        
        setUsuariosDisponibles(disponibles);
        
        if (disponibles.length > 0 && !disponibles.some(u => u.id === usuarioSeleccionado)) {
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        ...
                        {(usuariosDisponibles || []).length > 0 ? (
                            ...
                            <p className="no-users-message">Todos los usuarios están asignados a este personaje</p>
                        )}
                    </div>
